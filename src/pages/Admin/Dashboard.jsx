...
function UnsavedChangesModal({ isOpen, onConfirmDiscard, onKeepEditing, tLabel }) {
 ...
 {tLabel('संपादन जारी रखें', 'Keep Editing')}
 </button>
 <button type="button" className="admin-btn-danger" onClick={onConfirmDiscard}>
 {tLabel('परिवर्तन छोड़ें', 'Discard Changes')}
 </button>
 </div>
 </div>
 </div>
...
function AdminDashboard({ tab, initialSubTab }) {
 ...
 >
 2. {tLabel('जीवन यात्रा (टाइमलाइन)', 'Timeline')}
 </button>
 <button
 ...
 onClick={() => switchTab('contact', '/admin/sampark')}
 >
 1. {tLabel('संपर्क विवरण', 'Contact Info')}
 </button>
 <button
 ...
 <span className="admin-breadcrumb-title">
  {activeTab === 'home' && (
  homeSubTab === 'banner' || homeSubTab === 'hero' ? tLabel('होम > बैनर', 'Home > Banner') :
  homeSubTab === 'about' || homeSubTab === 'intro' ? tLabel('होम > परिचय', 'Home > About') :
  homeSubTab === 'poetry' || homeSubTab === 'featured' ? tLabel('होम > विशेष काव्य', 'Home > Featured Poetry') :
  tLabel('होम > विशेष पुस्तकें', 'Home > Featured Books')
  )}
  ...
  tLabel('कविताएं > कविता संपादित करें', 'Poetry > Edit Poem')
  )}
  {activeTab === 'publications' && (
  !paramId ? tLabel('पुस्तकें', 'Books') :
  paramId === 'new' ? tLabel('पुस्तकें > नई पुस्तक जोड़ें', 'Books > Add New Book') :
  tLabel('पुस्तकें > पुस्तक विवरण संपादित करें', 'Books > Edit Book Details')
  )}
  {activeTab === 'contact' && tLabel('संपर्क विवरण', 'Contact Info')}
  {activeTab === 'inbox' && tLabel('संदेश', 'Inbox')}
  {activeTab === 'settings' && tLabel('सेटिंग्स', 'Settings')}
  </span>
 </div>
 ...
 onClick={() => {
 window.dispatchEvent(new Event('adminFormCancel'))
 setIsDirty(false)
 }}
 style={{
...
function AboutManager({ about, initialSubTab, onUpdate, setIsDirty }) {
 ...
 useEffect(() => {
  ...
  if (about.photo_path) setPhotoPreview(getImageUrl(about.photo_path))
  }
  if (setIsDirty) setIsDirty(false)
  }
  window.addEventListener('adminFormCancel', handleCancel)
  return () => window.removeEventListener('adminFormCancel', handleCancel)
  }, [about, setIsDirty])
 ...
 {tLabel('प्रोफाइल फोटो', 'Profile Image')}
 </h4>
 ...
 <h4 style={{ margin: '0 0 12px 0', fontFamily: 'Lora, serif', color: 'var(--leona-charcoal)' }}>
 {tLabel('कवि विवरण', 'Author Details')}
 </h4>
 <div className="admin-form-grid">
 <div className="admin-form-group full-width">
 ...
 placeholder={tLabel('जैसे: कवि व लेखक परिचय', 'e.g. Poet & Author Profile')}
 required
 />
 </div>
 <div className="admin-form-group full-width">
 ...
 rows={3}
 placeholder={tLabel('संक्षिप्त परिचय...', 'Short summary...')}
 />
 </div>
 <div className="admin-form-group full-width">
 ...
 placeholder={tLabel('विस्तृत विवरण यहाँ लिखें...', 'Write full description here...')}
...
function PublicationsManager({ publications, onUpdate, setIsDirty }) {
 ...
 setItemsList(publications)
  ...
  window.addEventListener('adminFormCancel', handleCancel)
  return () => window.removeEventListener('adminFormCancel', handleCancel)
  }, [paramId, publications, itemsList, setIsDirty])
...
function PoemsManager({ poems, onUpdate, setIsDirty }) {
 ...
 setItemsList(poems)
  ...
  window.addEventListener('adminFormCancel', handleCancel)
  return () => window.removeEventListener('adminFormCancel', handleCancel)
  }, [paramId, poems, itemsList, setIsDirty])
 ...
 <div className="admin-form-group full-width">
 <label>{tLabel('संदर्भ / विवरण', 'Context / Description')}</label>
 <textarea
 className="admin-textarea"
 rows={2}
 maxLength={160}
 ...
 {adminLang === 'en' || adminLang === 'EN'
 ? `Lines: ${(formData.description || '').split('\n').length}/2 | Chars: ${(formData.description || '').length}/160`
 : `पंक्ति: ${(formData.description || '').split('\n').length}/2 | अक्षर: ${(formData.description || '').length}/160`
 }
 </span>
 </div>
 ...
 <div className="admin-form-group full-width">
 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
 <label style={{ margin: 0 }}>{tLabel('काव्य पंक्तियाँ *', 'Poem Text *')}</label>
 <button
 type="button"
 className="admin-btn-secondary"
 onClick={() => {
