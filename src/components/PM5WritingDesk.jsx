...
export default function PM5WritingDesk({ initialPages = [''], onSave = null, onClose = null, initialTitle = '', lang = 'hi', initialFullscreen = false }) {
 ...
 <span className="pm5-action-btn move" onClick={(e) => movePage(e, i, -1)} title={isEn ? 'Move Up' : 'ऊपर ले जाएं'}>{isEn ? 'Up' : 'ऊपर'}</span>
 <span className="pm5-action-btn move" onClick={(e) => movePage(e, i, 1)} title={isEn ? 'Move Down' : 'नीचे ले जाएं'}>{isEn ? 'Down' : 'नीचे'}</span>
 <span className="pm5-action-btn delete" onClick={(e) => deletePage(e, i)} title={isEn ? 'Delete Page' : 'पृष्ठ हटाएं'}>{isEn ? 'Delete' : 'हटाएं'}</span>
 </div>
